...
`demo/plugin.js` holds nine GLSL fragments and so does `source/Shaders.cpp`.
...
Nothing else can. `gvtest` drives the real plugin class through a real FFGL
...
reasoning that justifies the code: why the pedestal is subtracted at 4.5 sigma,
why the point limit is taken explicitly below a quarter of a sigma, why `max()`
in the trace pass would throw away every dwell dot.

The one transformation is a decode, not a normalisation. Four of these shaders
quote an identifier in a comment with backticks -- `Extent`, `sampleA`, `on`,
`half` -- and a backtick cannot appear raw inside a JavaScript template literal,
so `plugin.js` escapes it as \\`. This unescapes that and *rejects any other
backslash on the JS side*; there are none anywhere in the C++, so a second
escape could only be somebody hiding a difference.

------------------------------------------------------------------- what it cannot

Nothing here checks the *ported* half. `Tracer`, `Scanner`, `buildStream`,
`orderContours` and every `...FromParam` in plugin.js are a hand translation of
Tracer.cpp, Scanner.cpp, PointStream.cpp and Controls.cpp, and only a reader can
tell whether they still agree. When you change one of those, change it here too
...
#
# Nine and not seven: the trace stage is assembled at run time out of a shared
# constants string plus a body, because the vertex stage sizing the quad and the
# fragment stage subtracting the Gaussian's pedestal have to agree about
# `Extent`. The demo has to carry the pieces rather than the assembled strings,
# or a change to the constants would go unnoticed in both stages.
SHADERS = [
    ("VERTEX", "source/Shaders.cpp", "kVertexShader"),
    ("COPY", "source/Shaders.cpp", "kCopyShader"),
    ("EDGE", "source/Shaders.cpp", "kEdgeShader"),
    ("STABILISE", "source/Shaders.cpp", "kStabiliseShader"),
    ("DECAY", "source/Shaders.cpp", "kDecayShader"),
    ("BEAM_CONSTANTS", "source/Shaders.cpp", "kBeamConstants"),
    ("TRACE_VERTEX_BODY", "source/Shaders.cpp", "kTraceVertexBody"),
    ("TRACE_FRAGMENT_BODY", "source/Shaders.cpp", "kTraceFragmentBody"),
