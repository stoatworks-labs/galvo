...
void SimplifyPolyline( const std::vector< Vec2 >& in, float epsilon, bool closed, std::vector< Vec2 >& out )
{
	...
	else
	{
		...
		simplifyRun( in, 0, far, epsilon, keep );

		//The second half runs from `far` round to the first point again, which
		//is not in the vector twice -- so it is simplified against a copy with
		//the start appended.
		std::vector< Vec2 > tail( in.begin() + static_cast< std::ptrdiff_t >( far ), in.end() );
